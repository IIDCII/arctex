...
template <typename T> void fill_matrix(Matrix<T> *matrix, T value) {
...
template <typename T> bool matrix_filled(Matrix<T> *matrix, T value) {
  for (size_t i = 0; i < matrix->rows * matrix->columns; i++) {
...
template <typename T>
void add_into_result(Matrix<T> *matrix1, Matrix<T> *matrix2,
                     Matrix<T> *result) {
...
template <typename T>
[[nodiscard]] Matrix<T> *add_matrices_synchronously(Matrix<T> *matrix1,
                                                    Matrix<T> *matrix2) {
  if (!dimensions_equal(matrix1, matrix2))
    throw std::exception();
